namespace Unbound
{
    internal class GammaVisuals
    {
        ...
        {
            if (self?.owner?.OwnerRoom?.game?.session != null &&
                (self.owner.OwnerRoom.game.session.characterStats.name.value == "NCRunbound" ||
                self.owner.OwnerRoom.game.session.characterStats.name.value == "NCRtech"))
            {
                bool kochab = self.owner.OwnerRoom.game.session.characterStats.name.value == "NCRtech";
                    ...
                    for (int i = 0; i < (sLeaser.sprites[spr] as TriangleMesh).verticeColors.Length; i++)
                    {
                        ...
                        if (kochab)
                        {
                            ...
                            (sLeaser.sprites[spr] as TriangleMesh).verticeColors[i] = Color.Lerp(self.color,
                                Custom.HSL2RGB(0.87f, 0.39f, 0.33f), Mathf.InverseLerp(gradientStart, 1f, value));
                        }
                        else
                        {
                            float value = (float)i / (float)((sLeaser.sprites[spr] as TriangleMesh).verticeColors.Length - 1);
        ...
        public static void PupilcodeForGamma(On.OverseerGraphics.orig_InitiateSprites orig, OverseerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam)
        {
            ...
            if (self?.owner != null && self.overseer?.room != null &&
            ...
            else if (self?.owner != null && self.overseer?.room != null &&
                (self.overseer.room.world.game.session.characterStats.name.value == "NCRtech") &&
                self.overseer.PlayerGuide)
            {
                sLeaser.sprites[self.PupilSprite].color = new Color(1f, 0.1f, 0.1f, 0.5f);
        ...
        public static void EyeWhitesForGamma(On.OverseerGraphics.orig_DrawSprites orig, OverseerGraphics self, RoomCamera.SpriteLeaser sLeaser, RoomCamera rCam, float timeStacker, Vector2 camPos)
        {
            ...
            if (self?.owner?.room != null && self.overseer != null &&
                (self.owner.room.game.session.characterStats.name.value == "NCRunbound" ||
                self.owner.room.game.session.characterStats.name.value == "NCRtech") && self.overseer.PlayerGuide)
            {
                bool kochab = self.owner.room.game.session.characterStats.name.value == "NCRtech";
                sLeaser.sprites[self.WhiteSprite].color = Color.Lerp(self.ColorOfSegment(0.75f, timeStacker), kochab ?
                    new Color(0.47f, 0.29f, 0.23f) : new Color(0.2f, 0.56f, 0.47f), 0.5f);
                sLeaser.sprites[self.InnerGlowSprite].color = kochab ? new Color(0.89f, 0.85f, 0.85f) : new Color(0.23f, 0.25f, 0.28f);
            }
        ...
        public static Color GammaColouringSegments(On.OverseerGraphics.orig_ColorOfSegment orig, OverseerGraphics self, float f, float timeStacker)
        {
            ...
            self.overseer.room.world.game.session.characterStats.name.value == "NCRtech" && self.overseer.PlayerGuide)
            {
                return Color.Lerp(Color.Lerp(Custom.RGB2RGBA((self.MainColor + new Color(0.9f, 0.1f, 0.1f) +
                    self.earthColor * 8f) / 10f, 0.5f), Color.Lerp(self.MainColor, Color.Lerp(self.NeutralColor,
                    self.earthColor, Mathf.Pow(f, 2f)), 0.5f),
                    self.ExtensionOfSegment(f, timeStacker)), Custom.RGB2RGBA(self.MainColor, 0f),
                    Mathf.Lerp(self.overseer.lastDying, self.overseer.dying, timeStacker));
